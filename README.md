...
- **Chart reference** (`/chart`, `C` from home): the hard/soft/pair grids for the active table rules, color-coded per action with a legend.
  ...
  **A third tab prints the count you are being graded against.** Settings offers 58 counting systems and every counted card in the app is marked against one of their tag tables; until this tab the app printed those tags nowhere, so anyone who picked Zen or Wong Halves had to leave to learn what a 7 was worth.
  The tab shows the selected system's tags, its published description, and — since table rules decide a play and never what a card is worth — the system name and its balance in place of the S17/DAS/LS chips.
  Every figure is read back through `cardCountValue()`, the same accessor the counting engine sums a shoe with, so the table a trainee memorises cannot drift from what a miss is graded on: the same rule the strategy grids follow.
  Adjacent ranks whose tags agree share a column, which is how a published table prints it ("2–6 +1, 7–9 0, 10–A −1") and the only way thirteen ranks fit a phone.
  The merge is derived, never assumed — a system that tagged `J` apart from `10` would print `J` its own column — and a color-dependent system (Red Seven, KISS) gets a row each for red and black, with a rank joining a column only when both rows agree.
  **The tab says why two systems are drilled by different questions.** It sums a full deck of the tags on screen — derived, so the sentence and the table can never disagree — and a system that sums to 0 is one a true count can divide, while one that does not says which way its running count drifts and how far (`AceMT` is −20 a deck, not KO's +4).
  For an unbalanced system the app can cite a published schedule for, the IRC, key count, pivot, and insurance trigger follow for the shoe size the counting drill is set to; where there is none, the tab says so, since that absence is exactly why Settings offers that system no key-count drill.
  The strategy grids refuse to scroll sideways because a dragged cell loses the row and column it is read from; a tag strip keeps its meaning at any offset, so the handful of computer-only systems with a distinct weight per rank scroll here rather than crushing every playable system's table to fit them.
  On iOS the same table is a wrapping row of chips rather than columns, since the column count is the system's (three for one a human can play, ten for the computer-only ones) and a chip carries its own ranks, so it needs no column to be read under.
  **The tab is in the URL** (`/chart?tab=count`), which is what lets the two screens that talk about a counting system link to its tags: Settings, which picks among all 58 and printed the tags of none of them, and the counting drill's opening screen, which names the system in the last moment before the cards start coming.
  It also means the tab survives a reload and the back button, which the signal behind it never did; switching tabs replaces rather than pushes, so Back leaves the chart instead of stepping through whichever references were opened.
