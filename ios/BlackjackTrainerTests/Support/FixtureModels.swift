...
/// The fixture also carries `keyCountCases` and `betRampCases` for the archived
/// key-count and bet-spread drills; `Decodable` skips keys with no field, so
/// they decode away silently until those features return.
