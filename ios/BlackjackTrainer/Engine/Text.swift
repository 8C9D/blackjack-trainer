...
/// Mirrors the web `countOf`.
func countOf(_ value: Int, _ singular: String, display: String? = nil) -> String {
    "\(display ?? String(value)) \(value == 1 ? singular : "\(singular)s")"
}

func countOf(_ value: Double, _ singular: String, display: String? = nil) -> String {
    "\(display ?? String(value)) \(value == 1 ? singular : "\(singular)s")"
}
