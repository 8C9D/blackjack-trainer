...
### Post-roadmap continued: the count you are graded on, the cards you cannot see offline, and a bet the table misreported (2026-08-04)

Five slices, found the usual way - where does the app do something and never say
anything about it, and where do two surfaces tell the same story differently.

- **The app graded every counted card against tags it printed nowhere.** Settings offers 58 counting systems, each with its own per-rank tags, and every running count, true count, key-count call, bet and deck countdown in the app is marked against them - while the reference screen carried basic strategy and the deviation list and nothing about the count.
  A trainee who picked Zen or Wong Halves had to leave the app to learn what a 7 was worth.
  A third chart tab prints them, read back through `cardCountValue()` - the same accessor the counting engine sums a shoe with - so the table memorised cannot drift from what a miss is graded on, exactly as each strategy cell is the engine's own decision rather than a second transcription.
  Adjacent ranks whose tags agree share a column, which is how a published table prints it ("2-6 +1, 7-9 0, 10-A -1") and the only way thirteen ranks fit a phone; the merge is derived, never assumed, so a system tagging `J` apart from `10` would simply print `J` its own column, and a colour-dependent system (Red Seven, KISS) gets a row each for red and black with a rank joining a column only when both agree.
  The tab also says **why two systems are drilled by different questions**: it sums a full deck of the tags on screen - derived, so the sentence and the table can never disagree - and a system that sums to 0 is one a true count can divide, while one that does not says which way its running count drifts and how far (`AceMT` is -20 a deck, not KO's +4).
  Where a published key-count schedule exists it follows, resolved for the shoe size the drill is set to; where there is none the tab says so, since that absence is exactly why Settings offers that system no key-count drill.
  The page heading became **Chart** to match the button that opens it, now that it holds three references rather than one.
- **The installed app had no cards to show offline.** The card art sat in the service worker's `lazy` asset group with the rest of the images, so a fresh install cached none of it: install, board a plane, open a drill, and the shell loaded and the drill ran while every card was a blank rectangle with its alt text spilling over it.
  The hand could not be read, which is the whole of what a trainer shows - and the README claimed the app works offline.
  A deck is 53 files and ~1.9 MB (~650 KB over the wire), and that is simply the price of the claim; the worker registers only once the app is stable, so the fetch never competes with first paint.
  Reproduced end to end before the fix and again after, with a real worker and the network cut; `e2e/smoke/offline.e2e.ts` holds the line and skips rather than silently passing where no worker is registered, since only a production build registers one.
- **The backup file's payloads were half pinned.** `PracticeDay` had a stored-shape test on both platforms, written because a field added on one side and not the other is dropped silently on the trip - and the miss tally, which is the other structured payload the file carries, had none on either.
  Nor did `SessionStats`, which is the shape behind eight of the sixteen keys, nor the showdown tally, nor (on web) the count-drift wrapper the phone already pinned.
  All are pinned now on both platforms, plus the set of keys itself: a store one app writes and the other has never heard of survives the export and is thrown away on import, and the web half materialises the set through `PracticeDataService` rather than trusting a second hand-kept list.
  The two platforms agreed on every shape, so this closed a coverage gap rather than a live bug.
- **The table said what you should have bet, in a sentence that read as what you did.** A bet marked wrong announced "1 unit was the bet" - the called bet, in a headline only ever shown on a miss - while the chips actually out were 8, which the same screen says two lines above once the hand settles and says nothing about at all while it is still in progress.
  The bet-spread drill has always named both ("Your bet 8 units / Your spread says 1 unit"), so the same decision was being reported two different ways; the table now says "1 unit was the bet, not 8."
- **The tags were one tab away from the screens that choose them.** The chart's tab lived in a signal, so it could not be linked to or survive a reload; it is now `/chart?tab=count`, which lets Settings - the screen that picks among all 58 systems - and the counting drill's opening screen, which names the system in the last moment before the cards start coming, both link straight to the tags.
  Switching tabs replaces rather than pushes, so Back leaves the chart instead of stepping through whichever references were opened.
- **Validation.** +24 unit tests (1503), +18 Swift (596), +5 E2E (109), lint and format clean on both platforms, every web surface walked in the browser at desktop and 320px widths.
